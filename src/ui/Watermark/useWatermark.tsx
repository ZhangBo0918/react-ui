...
const getCanvasData = async (
  ...
  const drawText = () => {
    ...
    [...content].forEach((item, index) => {
      const { height: lineHeight, width: lineWidth } = measureSize.lineStyle[index];
...
function measureTextSize(ctx: CanvasRenderingContext2D, content: string[], rotate: number) {
  let width = 0,
    height = 0;
  const lineStyle: Array<{ width: number; height: number }> = [];
  ...
  content.forEach((item) => {
    ...
    height += textHeight;
    lineStyle.push({ height: textHeight, width: textWidth });
  });
